...
def compute_entity_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for e in entities:
        ...
        stats[e] = {
            ...
            "had_HST_prev": (e_mod in df.columns) and (df.loc[prev_mask, e_mod].astype(str).eq("High-Stakes Takedown").any()),
    ...
    for idx, row in df.iterrows():
        ...
        for e in entities:
            ...
            # Narrative Shaping
            if mod in ["High-Stakes Takedown", "Breakthrough Coverage"] or (prom >= 4.0 and outlet >= 4):
